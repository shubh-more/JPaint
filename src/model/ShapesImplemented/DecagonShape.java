...
public class DecagonShape implements InterfaceShape {
    ApplicationState appState;
    Shapes shapes;
    boolean isSelected;

    public DecagonShape(Shapes shapes) {
        this.shapes = shapes;
    }
    ...
    int[][] getPolygonArrays(int cx, int cy, int R) {
        int[] x = new int[10];
        int[] y = new int[10];
        double thetaInc = 2*Math.PI/10;
        double theta = (10 % 2 == 0) ? thetaInc : -Math.PI/2;
        for(int j = 0; j < 10; j++) {
