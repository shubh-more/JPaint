...
public class PentagonShape implements InterfaceShape {
    ApplicationState appState;
    Shapes shapes;
    boolean isSelected;

    public PentagonShape(Shapes shapes) {
        this.shapes = shapes;
    }
    ...
    public void draw(Graphics2D g) {
        ...
        int x = (int) shapes.endCoordinates.getA();
    ...
    int[][] getPolygonArrays(int cx, int cy, int R) {
        int[] x = new int[5];
        int[] y = new int[5];
        double thetaInc = 2*Math.PI/5;
        double theta = (5 % 2 == 0) ? thetaInc : -Math.PI/2;
        for(int j = 0; j < 5; j++) {
